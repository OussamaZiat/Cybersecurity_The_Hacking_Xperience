...
int main(int argc, char *argv[])
{
	...
	*(dev_name + strlen(dev_name) - 1) = '\0'; //the pcap_open_live don't take the last \n in the end
	...
	//we suppose that we do have a filter
	//in case of DNS filter we must add for example an expression like "port 53"
	//we do observe that we do have a " " between "port" and "53"
	//in argv it will be like we have two additionola arguments and not only one
	//we will then recreate the exp from the args
	if (argc >= 2) { // Case where filter
		...
		for (int i = 1; i < argc; i++) {
			printf("%s", filter_pointer);
			strcat(filter_exp,argv[i]);
			//add a " " after each arg value axcept the last one 
			if (i != argc - 1) {
				strcat(filter_exp," ");
			};
		};
		printf("Filter set : %s\n", filter_pointer);
		if (pcap_compile(handle, &fp, filter_exp, 0, net_ip) == -1) { // net_ip from pcap_lookupnet line 72
